...
-- SELECT * FROM (SELECT * ,
-- 		ROW_NUMBER() OVER (PARTITION BY customer_id ORDER BY customer_id, rownum2 DESC) AS rownum3
-- 		FROM (SELECT ROW_NUMBER() OVER (PARTITION BY customer_id, name ORDER BY customer_id) AS rownum2,
-- 				customer_rentals.rownum1,
-- 				customer_id,
-- 				customer_rentals.name
-- 				FROM 
-- 					(SELECT ROW_NUMBER() OVER (PARTITION BY customer_id ORDER BY customer_id, name) AS rownum1,
-- 						customer_id,
-- 						category.name
-- 					FROM rental
-- 					JOIN inventory ON inventory.inventory_id = rental.inventory_id
